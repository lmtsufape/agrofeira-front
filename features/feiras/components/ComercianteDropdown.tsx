...
import { Store, Search, ChevronDown, Loader2 } from "lucide-react";
...
export function ComercianteDropdown({
  ...
  const filtered = bancas.filter((b) =>
    b.comercianteNome.toLowerCase().includes(search.toLowerCase()),
  );

  const loadingText = loading
    ? "Carregando bancas..."
    : "Selecione o Comerciante / Banca";
      ...
      >
        <div className="flex items-center gap-3">
          {loading ? (
            ...
            <Store size={17} className="text-[#5bc48b]" />
          )}
          {placeholderText}
        </div>
        <ChevronDown
        ...
        <div className="absolute left-0 right-0 mt-2 rounded-xl overflow-hidden z-50 bg-white shadow-[0_8px_32px_rgba(0,61,4,0.15),0_0_0_1px_rgba(0,61,4,0.08)]">
          ...
          {filtered.length === 0 ? (
            ...
            <div className="max-h-[240px] overflow-y-auto">
              {filtered.map((b, i) => {
                const isSel = selected?.comercianteId === b.comercianteId;
