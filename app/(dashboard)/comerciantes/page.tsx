...
import { TableCellIcon } from "@/components/ui/TableCell";
  ...
  return (
    <div className="min-h-screen w-full flex flex-col bg-gradient-to-br from-[#f6faf4] to-[#edf5eb]">
      <main className="flex-1 px-4 md:px-6 py-6 max-w-7xl w-full mx-auto flex flex-col gap-6">
        <PageHeader
          title="Gerenciar Comerciantes"
          subtitle="Visualize, edite ou gerencie os comerciantes cadastrados no sistema"
          backHref="/dashboard"
        />
        ...
        {loading ? (
          <div className="flex items-center justify-center py-12">
            <p className="text-[#8aaa8d]">Carregando comerciantes...</p>
          </div>
        ) : error ? (
          ...
          <DataTableList
            data={filteredComerciantes}
            getKey={(comerciante) => comerciante.id}
            mobileHeaderTitle="Comerciantes"
            columns={[
              { label: "Nome", icon: Store, align: "left" },
              { label: "Ações", icon: Edit2, align: "right" },
            ]}
            renderRowDesktop={(comerciante) => (
              <>
                <TableCellIcon icon={Store} label={comerciante.nome} />
                <button
          ...
          <div className="flex items-center justify-center py-12 text-center">
            <p className="text-[#8aaa8d]">Nenhum comerciante encontrado</p>
          </div>
        )}
      </main>
    </div>
